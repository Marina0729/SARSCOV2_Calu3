...
```{r   GeTMM using TMM normalization on length corrected transcripts}
...
all_RF_norm_edgeR <- cpm(all_RF_counts )

plot_dist_RF <- all_RF_norm_edgeR %>%
  gather(sample, norm_RF_TPM, -Geneid) %>% 
  mutate(log2_norm_RF_TPM)
...
all_mRNA_norm_edgeR <- cpm(all_mRNA_counts )
