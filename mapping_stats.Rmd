...
mito_mRNA_norm <- calcNormFactors(mito_mRNA_norm, method = "RLE")
...
ggsave(filename = "results/boxplot_normalize_mRNA_RLE.png", plot = boxplot_normalize_mRNA, width = 20, height = 15, dpi = 600, units = "cm")
...
```{r   GeTMM using RLE normalization on length corrected transcripts for all genes}
...
all_RF_norm <- calcNormFactors(all_RF_norm, method = "RLE")
...
all_mRNA_norm <- calcNormFactors(all_mRNA_norm, method = "RLE")
...
write.csv(TE_table, "data/TE_table.csv")
...
TE_table_early <- TE_table %>%
  filter(str_detect(Geneid, "ENSG")) %>%
  dplyr::select(Geneid, s1, s2, s3, s4, s5, s6) %>%
  filter(s1<15, s2<15, s3<15, s4<15, s5<15, s6<15) %>% 
  filter(s1>0, s2>0, s3>0, s4>0, s5>0, s6>0) %>% 
  filter(s1!="NA", s2!="NA", s3!="NA", s4!="NA", s5!="NA", s6!="NA") %>% 
  filter(s1!="Inf", s2!="Inf", s3!="Inf", s4!="Inf", s5!="Inf", s6!="Inf")

plot_dist_TE_TPM <- TE_table_early %>%
  gather(sample, TE, -Geneid) %>% 
  ggplot(aes(x=TE, color = sample))+
  geom_density()
ggsave(filename = "results/plot_dist_TE_TPM.png", plot = plot_dist_TE_TPM, width = 15, height = 15, dpi = 300, units = "cm")

plot_boxplot_TE_TPM <- TE_table_early %>%
  gather(sample, TE, -Geneid) %>% 
  ggplot(aes(x=sample, y = TE))+
  geom_boxplot()+
  scale_y_log10()
ggsave(filename = "results/plot_boxplot_TE_TPM.png", plot = plot_boxplot_TE_TPM, width = 15, height = 15, dpi = 300, units = "cm")

library.sizes <- TE_table_early %>%
  gather(sample, TE, -Geneid) %>% 
  group_by(sample) %>% 
  summarise(sum = sum(TE))
...
modelling_TE_early <- voom(TE_counts_early, design = design_matrix_early, normalize.method = "none", plot = TRUE)
...
immune_genes_volcano <- immune_genes %>% 
  dplyr::rename(Geneid_stable = Geneid)

signif_immune_TE<- topTable(efit, number = 12287, p.value = 0.01, lfc = 1)  %>%
  dplyr::select(Geneid, logFC,adj.P.Val ) %>% 
  mutate(Geneid_stable = gsub("\\.\\d+$","", Geneid)) %>%
  left_join(immune_genes_volcano, by = "Geneid_stable") %>%
  filter(name != "NA")

coeff_TE <- topTable(efit, number = 12287) %>% 
  as.tibble() %>% 
  dplyr::select(genes, logFC, AveExpr, adj.P.Val) %>%
  mutate(genes = gsub("\\.\\d+$","", genes)) %>%
    mutate(genes = str_replace_all(genes, c("ENSG00000129514" = "FOXA1",
...
volcano_plot_TE_early <- EnhancedVolcano(coeff_TE,
    lab = rownames(coeff_TE),
    ...
    caption = "FC cutoff=2, adjusted p-value cutoff =0.01",
    FCcutoff = 1,
    pCutoff = 0.01)

ggsave(filename = "results/volcano_plot_TE_early.png", plot = volcano_plot_TE_early, width = 15, height = 15, dpi = 300, units = "cm")
...
mRNA_table_early <- read.csv("data/mRNA_CDS_counts.csv") %>%
  dplyr::select(-X) %>%
  left_join(mRNA_genes_Chr, by = "Geneid") %>%
  mutate(host = case_when(str_detect(Geneid, "ENSG") ~ TRUE, TRUE ~ FALSE )) %>%
  filter(host != "FALSE") %>%
  dplyr::rename(s1 = mRNA_CDS_s1, s2 = mRNA_CDS_s2, s3 = mRNA_CDS_s3, s4 = mRNA_CDS_s4, s5 = mRNA_CDS_s5, s6 = mRNA_CDS_s6) %>% 
  dplyr::select(Geneid, s1, s2, s3, s4, s5, s6) %>% 
...
coeff_plotting_mRNA <- topTable(efit_mRNA, number = 12183) %>% 
  as.tibble() %>% 
  dplyr::select(genes, logFC, AveExpr, adj.P.Val) %>%
  mutate(genes = gsub("\\.\\d+$","", genes)) %>%
  mutate(genes = str_replace_all(genes, c("ENSG00000196083" = "IL1RAP",
...
signif_immune_mRNA <- topTable(efit_mRNA, number = 12138, p.value = 0.01, lfc = 1) %>%
  mutate(genes = gsub("\\.\\d+$","", genes)) %>%
  left_join(immune_genes_volcano, by = "genes") %>%
  filter(name != "NA")

highlight_mRNA <- coeff_plotting_mRNA %>%
  as_tibble(rownames = "genes") %>% 
  filter(!str_detect(genes, "ENSG"))
...
volcano_plot_mRNA_early <- EnhancedVolcano(coeff_plotting_mRNA,
    ...
    xlim = c(-3.0, 3.0),
    ylim = c(0, 10),
    title = "Transcription efficiency \n 6hrs post infection",
    caption = "FC cutoff=2, adjusted p-value cutoff =0.01",
    FCcutoff = 1,
    pCutoff = 0.01)

ggsave(filename = "results/volcano_plot_mRNA_early.png", plot = volcano_plot_mRNA_early, width = 15, height = 15, dpi = 300, units = "cm")

#####################################################################

coeff_mRNA_rename <- topTable(efit_mRNA, number = 121387) %>%
  dplyr::rename(logFC.mRNA = logFC, adj.P.Val.mRNA = adj.P.Val) %>%
  dplyr::select(genes,logFC.mRNA, adj.P.Val.mRNA)

coeff_TE_rename <- topTable(efit, number = 12287) %>%
...
ggsave(filename = "results/plot_TE_versus_mRNA.png", plot = plot_TE_versus_mRNA, width = 15, height = 15, dpi = 300, units = "cm")
